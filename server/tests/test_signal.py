...
from comm_gateway.providers.signal import FakeSignalProvider, SignalProvider
...
def test_signal_provider_reply_http():
    calls = []
    ...
    def handler(request):
        ...
        assert body["method"] == "send"
        assert body["params"]["recipient"] == [SENDER]
        assert body["params"]["message"] == "reply HTTP"
        assert body["params"]["account"] == ACCOUNT
        assert body["params"]["quoteTimestamp"] == 1631458508784
        assert body["params"]["quoteAuthor"] == SENDER
        resp = {"jsonrpc": "2.0", "result": {"timestamp": 12345}, "id": "1"}
        return httpx.Response(200, json=resp)
    ...
    res = provider.reply(
        ACCOUNT,
        f"{SENDER}:1631458508784:{SENDER}",
        OutboundMessage(text="reply HTTP"),
    )
    assert len(calls) == 1
    assert res.provider_message_id == f"{SENDER}:12345:{ACCOUNT}"
    assert res.provider_thread_id == SENDER
