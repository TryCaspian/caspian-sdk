"""Microsoft Teams provider: Activity normalization and Bot Framework JWT checks."""
...
from comm_gateway.providers.base import Capability, OutboundMessage, WebhookVerificationError
from comm_gateway.providers.fakes.fake_teams import FakeTeamsProvider
...
def _jwt(private_key, *, aud=APP_ID, kid="kid1", exp_delta=300, channel_id="msteams") -> str:
    now = int(time.time())
...
def test_parse_activity_extracts_media_and_skips_non_messages():
    ...
    assert inbound.media[0]["url"] == "https://example.invalid/report.pdf"
    assert inbound.media[0]["mime_type"] == "application/pdf"
    assert inbound.media[0]["name"] == "report.pdf"
...
def test_parse_webhook_verifies_bearer_token_accept_and_reject():
    ...
    assert verifier.channel_id == "msteams"
...
def test_bot_framework_jwt_verifier_accepts_and_rejects_jwks_tokens():
    ...
    verifier._jwks_loaded_at = time.time()

    claims = verifier.verify(_jwt(private_key), APP_ID, "msteams")
    ...
    with pytest.raises(WebhookVerificationError, match="channel"):
        verifier.verify(_jwt(private_key, channel_id="wrong"), APP_ID, "msteams")
