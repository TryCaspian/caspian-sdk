...
class FakeTelegramProvider:
    ...
    def callback_payload(
        self, *, chat_id: int = 4242, message_id: int = 7, value: str = "reorder_123",
        sender_username: str = "customer",
    ) -> dict:
        """A Telegram callback_query update (a button tap), as the webhook delivers it."""
        self._update_seq += 1
        return {
            "update_id": 100_000 + self._update_seq,
            "callback_query": {
                "id": f"cbq{self._update_seq}",
                "from": {"id": 555_000 + self._update_seq, "username": sender_username,
                         "first_name": "Customer"},
                "message": {"message_id": message_id, "chat": {"id": chat_id, "type": "private"}},
                "data": f"caspian:{value}",
            },
        }

    def reaction_payload(
        self, *, chat_id: int = 4242, message_id: int = 7, emoji: str = "👍",
        sender_username: str = "customer",
    ) -> dict:
        """A Telegram message_reaction update (an emoji tapback)."""
        self._update_seq += 1
        return {
            "update_id": 100_000 + self._update_seq,
            "message_reaction": {
                ...
                "user": {"id": 555_000 + self._update_seq, "username": sender_username},
                "date": 1_752_400_000,
                "old_reaction": [],
    ...
    def photo_payload(
        self, *, chat_id: int = 4242, caption: str = "look", file_id: str = "photo123",
    ) -> dict:
        """A Telegram photo message (inbound media)."""
        self._update_seq += 1
        return {
            "update_id": 100_000 + self._update_seq,
            "message": {
                "message_id": self._update_seq,
                "from": {"id": 555_000 + self._update_seq, "username": "customer",
                         "first_name": "Customer"},
                "chat": {"id": chat_id, "type": "private"},
                "date": 1_752_400_000,
                "caption": caption,
