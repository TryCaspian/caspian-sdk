...
class FakeSlackProvider:
    ...
    # Interactivity (button taps). No signature check in the fake; the route +
    # ingest + event path is what these exercise.
    interaction_route_key = staticmethod(SlackProvider.interaction_route_key)

    def parse_interaction(self, payload, headers, credentials=None) -> list[InboundMessage]:
        from ..slack import _interaction_payload, parse_block_actions

        data = _interaction_payload(payload)
        if data is None:
            raise WebhookVerificationError("invalid interaction payload")
        return parse_block_actions(data)

    def block_actions_payload(self, *, channel="C123", ts="1.0", value="reorder_123",
                              action_id="caspian_btn_0", user="U456") -> bytes:
        """A form-encoded Slack block_actions body, as posted to the Interactivity URL."""
        ...
        payload = {
            ...
            "user": {"id": user, "username": "customer"},
            "channel": {"id": channel},
            "message": {"ts": ts},
            "trigger_id": f"trig{self._seq}",
            "actions": [{"action_id": action_id, "value": value,
                         "action_ts": f"{self._next()}.5"}],
        }
        return urlencode({"payload": json.dumps(payload)}).encode()

    def reaction_event(self, *, channel="C123", ts="1.0", emoji="thumbsup",
                       user="U456", added=True) -> dict:
        self._seq += 1
        return {
