...
def _extract_attachments(parsed) -> list[dict]:
    """Pull file attachments out of a parsed MIME message.

    Each becomes {"name", "mime_type", "size", "data": <base64>}. We inline the
    bytes as base64 so the attachment travels with the event with no extra store;
    the raw email already lives in S3 if a caller needs the original."""
    media: list[dict] = []
...
class SESEmailProvider:
    ...
    def _send_mime(
        ...
        for item in media or ():
            self._attach(mime, item)
        self._ses.send_email(Content={"Raw": {"Data": mime.as_bytes()}})
        return message_id

    @staticmethod
    def _attach(mime: EmailMessage, item: dict) -> None:
        """Attach one outbound media item. `data` is base64 bytes; `url` is fetched."""
        raw = item.get("data")
        if raw:
            content = base64.b64decode(raw)
        elif item.get("url"):
            content = httpx.get(item["url"], timeout=30).content
        else:
            return
    ...
    def parse_webhook(
        ...
        media = _extract_attachments(parsed)
