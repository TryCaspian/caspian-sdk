...
describe("CommClient", () => {
  ...
  it("connectEmail waits out provisioning then returns active", async () => {
    ...
    const { client, calls } = makeClient({
      "POST /v1/connections/email": () =>
        json({ id: "conn_1", status: "provisioning" }),
      "GET /v1/connections/conn_1": () => {
        polls += 1;
        return json({
          id: "conn_1",
          status: polls >= 2 ? "active" : "provisioning",
          address: "a@x",
        });
      },
    });
  ...
  it("connectSlack does not wait and returns an authorize_url", async () => {
    ...
    const conn = await client.connectSlack({
      slackClientId: "cid",
      slackClientSecret: "sec",
  ...
  it("throws CommError with detail on 4xx", async () => {
    ...
    await expect(
      client.connectX({ accessToken: "a", userId: "1" }),
  ...
  it("reply and sendMessage forward blocks in the request body", async () => {
    ...
    const blocks = [
      ...
      {
        type: "buttons",
        buttons: [{ label: "Track", url: "https://x/track" }],
      },
    ];
    ...
    expect(bodies[1]).toEqual({ text: null, html: null, blocks, attachments: null });
  });
  ...
  it("raises InsufficientCreditError on a 429 monthly_cap_reached body", async () => {
    const { client } = makeClient({
      "POST /v1/messages/m1/reply": () =>
        json(
          { detail: { reason: "monthly_cap_reached", message: "Capped." } },
          429,
        ),
    });
    const err = await client.reply("m1", "hi").catch((e) => e);
  ...
  it("billing methods hit the right endpoints with snake_case bodies", async () => {
    ...
    await client.setSpendLimits({
      ...
      channelCaps: { whatsapp: 5000 },
    });
    await client.setAutopay({
      thresholdCents: 500,
      topupCents: 2000,
      monthlyCapCents: 10000,
    });
    ...
    expect(limits?.body).toEqual({
      ...
      channel_caps: { whatsapp: 5000 },
    });
    const autopay = calls.find((c) => c.path === "/v1/billing/autopay");
    expect(autopay?.body).toMatchObject({
      enabled: true,
      threshold_cents: 500,
      topup_cents: 2000,
    });
  ...
  it("reply, sendMessage and react forward attachments / emoji", async () => {
    const bodies: any[] = [];
    const { client, calls } = makeClient({
      "POST /v1/messages/m1/reply": (req) =>
      ...
      "POST /v1/messages/m1/react": (req) =>
        req
          .json()
          .then((b) => (bodies.push(b), json({ ok: true, reacted: true }))),
    });
    const attachments = [
      { url: "https://x/i.png", mime_type: "image/png", name: "i.png" },
    ];
    await client.reply("m1", "here", null, null, attachments as any);
    await client.sendMessage("c1", null, null, null, attachments as any);
    ...
    expect(bodies[1]).toEqual({ text: null, html: null, blocks: null, attachments });
  ...
  it("a message carries received attachments to the handler", async () => {
    const { client } = makeClient({
      "GET /v1/events": (req) => {
        const after = Number(new URL(req.url).searchParams.get("after_seq"));
        if (after >= 1) return json([]);
        return json([
          {
            ...
            data: {
              message: {
                ...
                connection_id: "cn",
                text: "see attached",
                attachments: [{ name: "r.pdf", mime_type: "application/pdf" }],
              },
            },
          },
        ]);
