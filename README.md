...
- **`post_edit` (Native Streaming)**: On platforms that report the `edit_outbound` capability (like Discord, Slack, and Telegram), the SDK will post an initial message and continuously edit it as new chunks arrive, giving a true typing-like experience.
- **`final_only` (Fallback)**: On channels that do not report `edit_outbound` (like Email, SMS, or X/Twitter), or when the initial post fails, the SDK automatically buffers the stream and sends only the final complete message once the stream finishes, preventing fragmented or spammy messages.
